...
export const ProfileCompletionMeter: React.FC<ProfileCompletionMeterProps> = ({
  ...
  // Determine status color classes based on percentage
  ...
  return (
    <div className={`flex items-center space-x-4 bg-white p-4 rounded-xl border border-gray-200 border-solid shadow-xs ${className}`}>
      ...
      <div>
        <h4 className="text-xs font-bold text-gray-900">Profile Strength Index</h4>
        <p className="text-[10px] text-gray-400 font-bold mt-0.5 leading-tight">
          {percentage === 100
            ? 'Outstanding! Your resume profile is fully complete.'
            : 'Populate your skills, experience, and certificates to maximize hiring matches.'}
        </p>
      </div>
    </div>
  );
