      ...
      <div>
        <CardHeader className="mb-3">
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1 pr-2">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-md bg-kth-primary-50 border border-kth-primary-100 text-kth-primary-700 font-display font-extrabold flex items-center justify-center text-sm sm:text-base shrink-0">
              {companyLogo || company.charAt(0)}
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5 flex-wrap">
                <span className="text-xs font-semibold text-kth-slate-700 truncate max-w-[140px] sm:max-w-[180px]">{company}</span>
                {isVerified && <Badge variant="cyan" className="py-0.5 px-1.5 text-[9px] shrink-0">Verified</Badge>}
              </div>
          ...
          <button
            ...
            className={`p-2 rounded-md transition-colors shrink-0 min-h-[36px] min-w-[36px] flex items-center justify-center ${isSaved ? 'text-kth-primary-600 bg-kth-primary-50' : 'text-kth-slate-400 hover:text-kth-slate-700 hover:bg-kth-slate-100'}`}
          >
        ...
        <CardContent className="p-0">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 mb-3 sm:mb-4 flex-wrap">
            <Badge variant="mono" className="text-[11px] sm:text-xs">{salaryText}</Badge>
            <Badge variant="indigo" className="capitalize text-[11px] sm:text-xs">{employmentType.replace('_', '-')}</Badge>
            {matchScore && (
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {(Array.isArray(skills) ? skills : []).slice(0, 4).map((skill, idx) => (
              <span key={idx} className="px-2 py-0.5 rounded bg-kth-slate-100 text-kth-slate-700 text-[11px] sm:text-xs font-medium border border-kth-slate-200 truncate max-w-[150px]">
                {skill}
              </span>
            ))}
          </div>
